 ...
 * 
 * */
function move4(obj,attrs,duration,fx,endFn){
	//运动开始的时间
	var old = new Date();
	...
	for(var attr in attrs){
		j[attr] = {}
		if (parseFloat(getComputedStyle(obj)[attr]) == parseFloat(getComputedStyle(obj)[attr])) {
			//起始位置
			...
			var n = null;
			var m = null;
			for (var i = 0; i < getComputedStyle(obj)[attr].length; i++) {
				var s = getComputedStyle(obj)[attr].charCodeAt[i];
				if (s >= 48 && s <= 57) {
					n = i;
					break;
				}
			...
			for (var i = 0; i < attrs[attr].length; i++) {
				var s = attrs[attr].charCodeAt[i];
				if (s >= 48 && s <= 57) {
					m = i;
					break;
				}
			}
			console.log(getComputedStyle(obj)[attr].substring(n))
			j[attr].b =  parseFloat(getComputedStyle(obj)[attr].substring(n));
			j[attr].c = parseFloat(attrs[attr].substring(m)) - j[attr].b;
		}
	}
//	console.log(j);
